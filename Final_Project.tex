...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#creating a new variable, the percent of home wins for a team in a given year}
...
\CommentTok{#creating new variables that are the averages of ticket price, home wins, average attendance, and the percent of home wins}
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(sport }\OperatorTok{==}\StringTok{ "MLB"}\NormalTok{, }
\NormalTok{         year }\OperatorTok{>=}\StringTok{ }\DecValTok{2010}\NormalTok{, }
\NormalTok{         year }\OperatorTok{<=}\StringTok{ }\DecValTok{2012}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#demonstrating the use of both pivot_longer and pivot_wider}
\CommentTok{#our data was very clean to begin with so this was for the sake of practice and demonstrating the skill. }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Project Requirements }
\CommentTok{#1. pivot_longer: done}
\CommentTok{#2. pivot_wider: done}
\CommentTok{#3. group_by: Done}
\CommentTok{#4. summarize: Done}
\CommentTok{#5. filter: Done}
\CommentTok{#6. select: Done}
\CommentTok{#7. mutate: Done }
\CommentTok{#8. one table: Done}
\CommentTok{#9. two visualization: Done}
\CommentTok{#10. inline code: Done}
\end{Highlighting}
\end{Shaded}
