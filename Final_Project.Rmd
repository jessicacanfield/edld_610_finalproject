...
knitr::opts_chunk$set(echo = TRUE, 
                      warning = FALSE,
                      message = FALSE)
#I deleted the library for dplyr because it is already included in the tidyverse :) 
library(papaja)
library(kableExtra)
library(here)
library(rio)
...
mlb <- import(here("Data", "MLB.xlsx")) %>%
  ...
  as_tibble()  %>% 
  mutate(attend_tot = as.numeric(attend_tot),
         attend_avg = as.numeric(attend_avg), 
         capacity = as.numeric(capacity), 
         games = as.numeric(games), 
         ticket_price = as.numeric(ticket_price),
         home_wins = as.numeric(home_wins)) #changed attent_tot, games, home_wins, ticket price and attend_avg to numeric to try to get bind to work, also the 'numeric' code gave me an error, I assumed that we wanted this variable to be numeric so I changed it to 'as.numeric' + combined it with the other mutates :) 
str(mlb) #I added code to see the structure of each sports dataset when I was trying to figure out why it wouldn't bind, feel free to remove! 
...
nba <- nba %>% 
  mutate(capacity = as.numeric(capacity), 
         ...
         games = as.numeric(games), 
         ticket_price = as.numeric(ticket_price), 
         home_wins = as.numeric(home_wins)) #put each mutated variable on diff. line for readability when I was trying to figure out which needed to be changed from character form for bind 
str(nba)
...
sports_rev %>%
  ...
  labs(x= "Average Ticket Price", y="Average Attendance", title="whatever title you want!", color="Sport")
  theme_minimal() #added labels, left the title blank so you could change it to whatever! 
...
sports_rev %>%
  ...
  labs(x= "Average Home Wins", y="Average Attendance", title="whatever title you want!") +
  theme_minimal() #added labels and changed theme to minimal to match above 
```
